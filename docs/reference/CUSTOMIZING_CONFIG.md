...
| Deep mapping merge | Traffic, spawn, proxy, site map, auth, observation, timing, HTTP, storage | Nested mappings merge; scalar values replace; lists append. |
...
Fields omitted from an overlay remain packaged defaults. `_replace` is not a deletion language.
