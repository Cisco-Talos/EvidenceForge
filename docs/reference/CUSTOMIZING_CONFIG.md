...
| Causal/source-native timing | `timing_profiles.yaml` (`relationships` for causal prerequisites, source latency, teardown margins, Zeek analyzer offsets, SSH syslog subevent timing, TLS duration floors, multi-sensor Zeek timestamp offsets, plus Windows/Sysmon collision spacing) |
