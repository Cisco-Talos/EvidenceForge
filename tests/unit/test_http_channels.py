...
@pytest.mark.parametrize(
    ...
    [
        ...
        "_reserved_affinity_digests",
        "_integrity_token",
    ],
)
def test_in_place_manager_token_tamper_is_rejected_and_releases_original_reservations(
    field_name: str,
) -> None:
    """Every manager target and the exact nested common capability are authenticated."""
    ...
    assert isinstance(transport, HttpChannelTransport)
    if field_name == "kind":
        object.__setattr__(token, field_name, "reuse")
    elif field_name == "application_token":
        object.__setattr__(token, field_name, replace(token.application_token))
    elif field_name == "nested_application":
        object.__setattr__(token.application_token, "replacement_reason", "tampered")
    elif field_name == "result":
        object.__setattr__(token, field_name, replace(transport, conn_id="tampered"))
    elif field_name == "expected_transport":
        object.__setattr__(token, field_name, transport)
    elif field_name == "prepared_transport":
        object.__setattr__(
            token,
            field_name,
            replace(transport, zeek_uid="tampered"),
        )
    elif field_name in {"_manager_token", "_reservation_id", "_owner_shard_id"}:
        object.__setattr__(token, field_name, getattr(token, field_name) + 1)
    elif field_name == "_owner_id":
        object.__setattr__(token, field_name, "tampered-owner")
    elif field_name == "_reserved_channel_ids":
        object.__setattr__(token, field_name, ())
    elif field_name == "_reserved_affinity_digests":
        object.__setattr__(token, field_name, ())
    else:
        object.__setattr__(token, field_name, f"{getattr(token, field_name)}tampered")
...
def test_tamper_inside_claim_aborts_before_common_or_sidecar_publication(nested: bool) -> None:
    """Body-time mutation cannot redirect the already claimed composite write."""
    ...
    transport = token.result
    assert isinstance(transport, HttpChannelTransport)
    with pytest.raises(StateError, match="integrity|authentication"):
        with manager.prepared_admission(token) as prepared:
            if nested:
                object.__setattr__(token.application_token, "replacement_reason", "tampered")
            else:
                object.__setattr__(token, "result", replace(transport, conn_id="tampered"))
            prepared.commit_no_fail()
    assert manager.channel_snapshot(transport.channel_id) is None
    assert manager.get_transport(transport.channel_id) is None
    assert not manager.authenticates_admission_token(token)
