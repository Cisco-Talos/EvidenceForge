...
def test_pack_identity_is_validated_before_authoring_paths(
    ...
    with pytest.raises(PackError, match="invalid pack identity"):
        repository.create_skeleton("industry", name, version)
...
def test_pack_authoring_rejects_symlinked_repository_ancestry(tmp_path: Path) -> None:
    ...
    with pytest.raises(PackError, match="cannot contain a symlink"):
        repository.create_skeleton("industry", "safe-name", "1.0.0")
...
def test_pack_authoring_never_replaces_an_existing_empty_destination(tmp_path: Path) -> None:
    ...
    destination = tmp_path / ".eforge" / "packs" / "industry" / "existing-pack" / "1.0.0"
    destination.mkdir(parents=True)
    repository = PackRepository(tmp_path)

    with pytest.raises(PackError, match="already exists"):
        repository.create_skeleton("industry", "existing-pack", "1.0.0")
...
def test_pack_include_escape_is_rejected_before_outside_file_is_read(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "contained-pack", "1.0.0")
    ...
    with pytest.raises(PackError, match="escapes allowed root") as exc_info:
        repository.resolve(
            PackReference(source="project", name="contained-pack", version="1.0.0"),
...
def test_path_cli_entrypoints_expand_contained_manifest_includes(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "included-cli", "1.0.0")
...
def test_pack_semantic_documents_share_one_cumulative_include_budget(
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "bounded-semantic", "1.0.0")
    ...
    with pytest.raises(PackError, match="include bytes exceed limit"):
        repository.resolve(
            PackReference(source="project", name="bounded-semantic", version="1.0.0"),
            expected_type="industry",
        )
...
def test_pack_tree_byte_budget_includes_nonsemantic_companions(
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "bounded-companion", "1.0.0")
    ...
    with pytest.raises(PackError, match="pack bytes exceed limit"):
        repository.resolve(
            PackReference(source="project", name="bounded-companion", version="1.0.0"),
            expected_type="industry",
        )
...
def test_pack_tree_bounds_depth_file_count_and_entry_count(
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "bounded-tree", "1.0.0")
    ...
    with pytest.raises(PackError, match="entry count exceeds limit"):
        repository.resolve(
            PackReference(source="project", name="bounded-tree", version="1.0.0"),
            expected_type="industry",
        )
...
def test_pack_copy_rechecks_immutable_snapshot_budget_before_writing(
    ...
    repository = PackRepository(tmp_path)
    repository.create_skeleton("industry", "copy-budget-source", "1.0.0")
    source = repository.resolve(
        PackReference(source="project", name="copy-budget-source", version="1.0.0"),
        expected_type="industry",
    )
    ...
    with pytest.raises(PackError, match="pack bytes exceed limit"):
        repository.copy(source, name="copy-budget-destination", version="1.0.0")
...
def test_pack_digest_and_assets_use_the_exact_bytes_parsed_for_validation(
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "captured-load", "1.0.0")
    ...
    pack = repository.resolve(
        PackReference(source="project", name="captured-load", version="1.0.0"),
        expected_type="industry",
    )
...
def test_pack_copy_uses_the_resolved_immutable_source_snapshot(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "captured-copy", "1.0.0")
    storage_path = root / "catalogs" / "storage_catalog.yaml"
    ...
    source = repository.resolve(
        PackReference(source="project", name="captured-copy", version="1.0.0"),
        expected_type="industry",
    )

    storage_path.write_bytes(mutated_storage)
    readme_path.write_bytes(mutated_readme)
    destination = repository.copy(source, name="captured-copy-fork", version="1.1.0")
...
def test_pack_authoring_rolls_back_a_failed_post_publish_reload(
    ...
    with pytest.raises(PackError, match="injected reload failure"):
        repository.create_skeleton("organization", "rollback-org", "1.0.0")
...
def test_copy_rolls_back_a_semantically_invalid_destination(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    source_root = repository.create_skeleton("industry", "invalid-source", "1.0.0")
    _write_yaml(
    ...
    source = repository.resolve(
        PackReference(source="project", name="invalid-source", version="1.0.0"),
        expected_type="industry",
    )

    with pytest.raises(PackError, match="missing export"):
        repository.copy(source, name="invalid-copy", version="1.0.0")
...
def test_copy_flattens_manifest_includes_without_leaving_orphan_yaml(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    source_root = repository.create_skeleton("industry", "included-manifest", "1.0.0")
    ...
    source = repository.resolve(
        PackReference(source="project", name="included-manifest", version="1.0.0"),
        expected_type="industry",
    )
    destination = repository.copy(source, name="flattened-copy", version="1.1.0")
    ...
    copied = repository.resolve(
        PackReference(source="project", name="flattened-copy", version="1.1.0"),
        expected_type="industry",
    )
...
def test_catalog_field_origins_retain_qualified_include_paths(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "origin-pack", "1.0.0")
    ...
    pack = repository.resolve(
        PackReference(source="project", name="origin-pack", version="1.0.0"),
        expected_type="industry",
    )

    assert (
        pack.catalog_field_origins["storage_catalog.origin-pack:records.data.files.0.mime"]
...
def test_path_pack_copy_records_source_digest_and_location(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "path-source", "1.0.0")
    source = repository.resolve(
        PackReference(
            source="path",
            path=str(root),
    ...
    destination = repository.copy(source, name="path-copy", version="1.1.0")
    provenance = (destination / "COPY_PROVENANCE.md").read_text(encoding="utf-8")

    assert "Source reference: `path:industry:path-source@1.0.0`" in provenance
...
def test_manifest_include_path_dependency_survives_validation_compile_and_copy(
    ...
    repository = PackRepository(tmp_path)
    dependency_root = repository.create_skeleton("industry", "path-industry", "1.0.0")
    source_root = repository.create_skeleton("organization", "path-org", "1.0.0")
    manifest_path = source_root / "pack.yaml"
    manifest = yaml.safe_load(manifest_path.read_text(encoding="utf-8"))
    ...
    source = repository.resolve(
        PackReference(source="project", name="path-org", version="1.0.0"),
    ...
    destination = repository.copy(source, name="copied-path-org", version="2.0.0")
    copied_manifest = yaml.safe_load((destination / "pack.yaml").read_text(encoding="utf-8"))
    ...
    copied = repository.resolve(
        PackReference(source="project", name="copied-path-org", version="2.0.0"),
        expected_type="organization",
    )
...
def test_copy_rewrites_only_typed_exact_self_references(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    source_root = repository.create_skeleton("organization", "source-org", "1.0.0")
    _write_yaml(
    ...
    _write_yaml(
        source_root / "catalogs/application_catalog.yaml",
        {
            "application_catalog": {
                "portal": {
                    "description": "source-org:portal",
                    "data": {
                        "personas": ["source-org:operator"],
                        "processes": ["source-org:portal-processes"],
                        "connections": {
                            "primary": {
                                "destination": "source-org:portal-destination",
                                "service": "web",
                            }
                        },
                    },
        ...
        source_root / "catalogs/traffic_catalog.yaml",
        {
            "traffic_catalog": {
                "portal-traffic": {
                    ...
                    "data": {
                        "audience": ["source-org:operator"],
                        "applications": [
                            {"application": "source-org:portal", "connection": "primary"}
                        ],
                        "outbound": [
                            {
                                "port": 443,
    ...
    source = repository.resolve(
        PackReference(source="project", name="source-org", version="1.0.0"),
        expected_type="organization",
    )

    destination = repository.copy(source, name="renamed-org", version="2.0.0")
    ...
    assert application["description"] == "source-org:portal"
    assert application["data"]["personas"] == ["renamed-org:operator"]
    assert application["data"]["processes"] == ["renamed-org:portal-processes"]
    assert application["data"]["connections"]["primary"]["destination"] == (
        "renamed-org:portal-destination"
    )
    assert traffic["audience"] == ["renamed-org:operator"]
    assert traffic["applications"][0]["application"] == "renamed-org:portal"
    assert traffic["outbound"][0]["dns_tags"] == ["renamed-org:portal"]
    assert destination_catalog["tags"] == ["renamed-org:portal"]
    assert environment["users"][0]["persona"] == "renamed-org:operator"
    assert (destination / "COPY_PROVENANCE.md").is_file()

    before = repository.resolve(
        PackReference(source="project", name="renamed-org", version="2.0.0"),
        expected_type="organization",
    ).digest
...
def test_organization_loader_qualifies_local_environment_references(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("organization", "local-org", "1.0.0")
    _write_yaml(
        root / "catalogs/persona_catalog.yaml",
        {
            "persona_catalog": {
    ...
    loaded = repository.resolve(
        PackReference(source="project", name="local-org", version="1.0.0"),
        expected_type="organization",
    )

    assert loaded.environment["users"][0]["persona"] == "local-org:operator"
    assert loaded.environment["storage"]["servers"][0]["shares"][0]["preset"] == "local-org:records"
...
def test_copy_reference_rewriter_covers_organization_storage_presets() -> None:
    ...
    document = {
        "environment": {
            "description": "source-org:records",
            "storage": {
                "servers": [
                    {
                        "shares": [
                            {
                                "preset": "source-org:records",
                                "description": "source-org:records",
                            }
                        ]
                    }
    ...
    changed = _rewrite_pack_self_references(
        document,
        old_name="source-org",
        new_name="renamed-org",
    )

    share = document["environment"]["storage"]["servers"][0]["shares"][0]
    assert changed is True
    assert share["preset"] == "renamed-org:records"
    assert share["description"] == "source-org:records"
    assert document["environment"]["description"] == "source-org:records"
...
def test_copy_reference_rewriter_covers_baseline_persona_audiences() -> None:
            ...
            "traffic_affinities": [
                {
                    "name": "portal",
                    "audience": {
                        "personas": ["source-org:operator", "dependency:analyst"],
                    },
                }
            ],
            "traffic_suppression": [
                {"audience": {"personas": ["source-org:operator"]}, "factor": 0.5}
            ],
            "description": "source-org:operator",
        }
    }
    ...
    assert baseline["traffic_affinities"][0]["audience"]["personas"] == [
        "renamed-org:operator",
        "dependency:analyst",
    ]
    assert baseline["traffic_suppression"][0]["audience"]["personas"] == ["renamed-org:operator"]
    assert baseline["description"] == "source-org:operator"
...
def test_pack_init_and_copy_have_stable_json_contracts(tmp_path: Path) -> None:
    ...
    copied = runner.invoke(
        ...
        [
            "pack",
            "copy",
            "project:industry:source-pack@1.0.0",
            "--name",
            "copied-pack",
            "--version",
            "1.1.0",
...
def test_pack_list_failure_has_a_stable_json_envelope(
    ...
    assert result.exit_code == 1
    assert payload == {"packs": [], "error": "injected inventory failure"}
    assert "Error:" not in result.stdout
...
def test_pack_validate_normalizes_include_errors_as_json(tmp_path: Path) -> None:
    ...
    repository = PackRepository(tmp_path)
    root = repository.create_skeleton("industry", "broken-include", "1.0.0")
    _write_yaml(
        root / "catalogs/persona_catalog.yaml",
    ...
    result = runner.invoke(
        ...
        [
            "pack",
            "validate",
            "project:industry:broken-include@1.0.0",
            "--project-root",
            str(tmp_path),
            "--json",
        ],
