...
from evidenceforge.models.scenario import (
    Scenario, Environment, TimeWindow, BaselineActivity, OutputSpec, Persona,
)
...
def _make_scenario(personas=None, intensity="medium"):
    ...
    return Scenario(
        ...
        environment=Environment(
            ...
            users=[
                User(username="user.one", full_name="User One", email="u1@t.com",
                     enabled=True, persona="developer", primary_system="WKS-01"),
            ],
            systems=[
                System(hostname="WKS-01", ip="10.0.10.1", os="Windows 10", type="workstation"),
            ],
        ),
        baseline_activity=BaselineActivity(description="Test", intensity=intensity, variation="low"),
...
class TestSoftRamp:
    ...
    def test_before_work_near_zero(self):
        ...
        events_h6 = [engine._calculate_events_for_hour(user, current_hour=6, persona=persona)
                     for _ in range(50)]
    ...
    def test_ramp_up_before_start(self):
        ...
        events_h8 = [engine._calculate_events_for_hour(user, current_hour=8, persona=persona)
                     for _ in range(50)]
    ...
    def test_full_activity_during_work(self):
        ...
        events_h13 = [engine._calculate_events_for_hour(user, current_hour=13, persona=persona)
                      for _ in range(50)]
    ...
    def test_lunch_soft_dip(self):
        ...
        events_lunch = [engine._calculate_events_for_hour(user, current_hour=12, persona=persona)
                        for _ in range(100)]
        events_work = [engine._calculate_events_for_hour(user, current_hour=14, persona=persona)
                       for _ in range(100)]
    ...
    def test_evening_tail(self):
        ...
        events_h17 = [engine._calculate_events_for_hour(user, current_hour=17, persona=persona)
                      for _ in range(50)]
    ...
    def test_peak_hours_still_higher(self):
        ...
        events_normal = [engine._calculate_events_for_hour(user, current_hour=9, persona=persona)
                         for _ in range(100)]
        events_peak = [engine._calculate_events_for_hour(user, current_hour=10, persona=persona)
                       for _ in range(100)]
        avg_normal = sum(events_normal) / len(events_normal)
        avg_peak = sum(events_peak) / len(events_peak)
        assert avg_peak > avg_normal, f"Peak ({avg_peak:.1f}) should exceed normal ({avg_normal:.1f})"
...
class TestWorkHourMultiplierDirect:
    ...
    def test_user_offsets_shift_timing(self):
        ...
        early_offsets = {'start_offset': -0.5, 'end_offset': 0, 'lunch_start_offset': 0, 'lunch_duration_offset': 0}
        # Late starter: starts 30min later
        late_offsets = {'start_offset': 0.5, 'end_offset': 0, 'lunch_start_offset': 0, 'lunch_duration_offset': 0}
        ...
        assert m_early_h8 > m_late_h8, f"Early ({m_early_h8:.2f}) should > late ({m_late_h8:.2f}) at hour 8"
...
class TestActivityClusters:
    ...
    def test_persona_affects_cluster_size(self):
        ...
        dev_gaps = [(dev_times[i+1] - dev_times[i]).total_seconds() for i in range(len(dev_times) - 1)]
        exec_gaps = [(exec_times[i+1] - exec_times[i]).total_seconds() for i in range(len(exec_times) - 1)]
