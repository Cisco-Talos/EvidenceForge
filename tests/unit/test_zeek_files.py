    ...
    def test_emitter_output_fields(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            emitter.emit_event({
                ...
                'source': 'HTTP', 'depth': 0,
                'analyzers': [], 'mime_type': 'text/html',
                'duration': 0.005, 'local_orig': False,
                'is_orig': False, 'seen_bytes': 1024,
                'total_bytes': 1024, 'missing_bytes': 0,
                'overflow_bytes': 0, 'timedout': False,
            })
            emitter.close()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            event = SecurityEvent(
                ...
                network=NetworkContext(
                    ...
                    dst_port=80, protocol='tcp', zeek_uid='CConnUID12345678',
                ),
                file_transfer=FileTransferContext(
                    fuid='FFileUID12345678', source='HTTP', seen_bytes=100,
                ),
            )
            emitter.emit(event)
            emitter.close()
