...
- Note that the causal expansion engine auto-generates prerequisite events (DNS lookups before connections, Kerberos/DC-bundle TGT/TGS evidence before domain logons, audit events from command patterns, etc.) — these appear in the logs but are not explicitly listed in the scenario YAML
