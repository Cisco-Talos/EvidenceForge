...
description: >
  ...
  defaults, and related baseline-generation catalogs. Use for explicit project-overlay requests such
  as "check my config", "change the DNS registry", or "tune application_catalog.yaml". Before a
  generic request to add a persona, domain, application, traffic pattern, or storage vocabulary,
  determine whether it belongs in one scenario, an industry pack, an organization pack, or the
  project overlay. Do not use this skill to author scenarios or packs, run generation, or change
  engine-owned safety, evaluation, output-format, resource, runtime, or OOB policy.
---
