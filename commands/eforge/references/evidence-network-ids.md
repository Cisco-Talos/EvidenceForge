...
| files.log | `files.json` | Visible HTTP, SMTP, SMB, and OCSP file analysis with directional hosts, conn UID, optional name/MIME, counts, and hashes. |
| smb_mapping.log | `smb_mapping.json` | One sparse visible mapping for a successful share tree. |
| smb_files.log | `smb_files.json` | Sparse successful OPEN/READ/WRITE/RENAME/DELETE; FUID only when file analysis exists. |
...
capture loss rather than a real PCAP counter.
