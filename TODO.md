...
- [ ] **P1** Windows 4624 ElevatedToken realism — latest data-only blind review found all rendered 4624 events had `ElevatedToken=%%1842` (740/740), including ordinary users, services, machine accounts, and anonymous logons. This is not primarily a timing issue but should be fixed near the Windows auth audit because the value should come from auth/session privilege semantics rather than a uniform renderer default.
- [ ] **P1** Anonymous logon field realism — latest data-only blind review found anonymous Type 3 logons rendering unlikely identity/source fields: `TargetDomainName=ACME`, `IpAddress=-`, local workstation name, and elevated token. Review anonymous-auth generation/context so anonymous logons use source-native domain/source/elevation semantics.
