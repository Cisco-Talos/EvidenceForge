...
- [ ] TLS certificate chain depth realism — blind review still flags every x509 observation as leaf-only (`cert_chain_fuids` length 1 and no `basic_constraints.ca=true`). Implement a chain-aware internal event model so ssl.log can reference leaf + intermediate FUIDs and x509.log can emit corresponding CA/intermediate records without breaking per-sensor FUID derivation.
