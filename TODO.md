...
- [ ] **IN PROGRESS** Loop 40 iteration-test assessment continuation — implemented root-cause fixes for the loop-39 P0/P1/P2 findings before regeneration: Zeek multi-sensor observation variance now preserves `*_ip_bytes >= *_bytes + packet overhead` invariants; ICMP Zeek conn rows render source-native type/code ports and echo request/reply history; Zeek files.log transfer rows are delayed after their referenced connection start; Office reading-location registry `Datetime` values are materialized before the Event 13 timestamp; eCAR failed logons carry an `attempt_failed` session lifecycle qualifier; Sysmon ProcessGuid timestamp segments no longer expose tiny boot-relative counters; and the data-driven dsquery command pool now varies query targets and limits. Verification passed: `eforge validate-config`, focused regression tests, full normal `uv run pytest -v` (2780 passed, 37 skipped), Ruff check, and Ruff format check. Regeneration, eval, and blind review are pending.
