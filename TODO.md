...
- [ ] Server-originated proxy User-Agent destination/OS refinement — loop31 blind review found DMZ server proxy traffic no longer uses desktop browser UAs, but package-manager UAs can appear against unrelated SaaS/CDN domains and Fedora `libdnf` can appear from Ubuntu hosts. Refine server proxy UA selection so package managers are OS-appropriate and limited to package/update repositories, while generic SaaS/API destinations use clients such as curl, Wget, Python requests, Go, Java, or app-specific agents.
