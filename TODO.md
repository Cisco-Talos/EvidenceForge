...
- [ ] **P1** Cross-source network timestamp profile — Windows Security 5156, Zeek conn, and eCAR FLOW can share exact tuple timestamps for the same connection. Decide and implement a source-native timestamp offset/profile layer so exact equality is reserved for sources that truly share the same capture timestamp.
- [ ] **P2** Proxy domain-class path/content profile completion — inspected proxy GET rows can still pair vendor/update user agents and hosts with generic browser paths such as `/login` or `/favicon.ico`. Extend domain-class-aware HTTP path selection across proxy/web/Zeek HTTP contexts.
...
- [ ] **P0** Post-logoff same-LogonID lifecycle contradictions — latest data-only blind review found visible 4634 logoff followed by later 4688 process activity and later Type 7 unlock/4624 for the same host + LogonID. Named examples: admin01 LogonID `0xa8eddd19` had 4634 at 16:05:09, 4688 `taskhostw.exe` at 16:08:55, then Type 7 4624 at 16:49:00; ws03 LogonID `0x120d7c6a` had 4634 at 16:29:34, later Chrome/PowerShell 4688 around 16:49, then Type 7 4624 at 17:03:00. Treat this as a first-class timing-audit case: no dependent process/unlock activity should render after a visible final logoff for that session unless a new session identifier is used.
...
- [ ] **P0** IDS DNS alert/query contradiction — follow-up review of `/private/tmp/eforge-timing-loop6-output/data` found Snort DNS alerts whose message described one query class while Zeek DNS for the same UDP tuple rendered a different query, e.g. an `ET DNS Query for .to TLD` alert on a tuple whose Zeek `dns.json` query was a generated reverse-style hostname. Audit DNS/IDS context construction so IDS DNS signatures and Zeek DNS payloads derive from one canonical DNS query on the `SecurityEvent`.
- [ ] **P0** Timestamp compression bursts — follow-up review of `/private/tmp/eforge-timing-loop6-output/data` found hundreds of Windows Security/Sysmon events compressed into a few hundred microseconds on a single host. Audit burst-generation and rendered timestamp profiles so dense activity clusters retain realistic within-cluster spacing without breaking causal ordering.
