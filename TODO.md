...
- [ ] **IN PROGRESS** Loop 37 iteration-test assessment continuation — fixing loop-36 root-cause findings. Implemented fix set 1: Windows Security 5156 no longer falls back to `ProcessID=4`/`Application=System` for ordinary flows with unknown process ownership, explicit successful TCP connections now choose varied Zeek `SF` histories instead of collapsing to `ShADadfF`, and web-scan IDS rate alerts now use a variable seeded gate instead of a fixed near-one-minute cadence. Verification so far: focused tests, full normal `uv run pytest -v`, Ruff check, and Ruff format check passed. Remaining loop-37 review targets after regeneration/review: Type 3/10 self-source logons and PSEXESVC child-process logon semantics if still reproduced.
