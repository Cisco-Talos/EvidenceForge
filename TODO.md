...
- [ ] **IN PROGRESS** HTTP/proxy blind review critical/high follow-up — fixed denied/inspected HTTPS proxy semantics and local proxy CONNECT timestamp ordering. Implementing OCSP option 1: issuer-aware OCSP responder config plus supporting HTTP/files evidence so `ocsp.id` joins to `files.fuid` and the file row has a visible `conn_uids` link. Broader cross-emitter timing should use a shared event-timing helper/context in a later pass if additional concrete ordering bugs appear.
