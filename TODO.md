...
- [ ] **IN PROGRESS** Loop 43 realism fixes — address loop-42 concrete root causes: preserve Zeek cleartext HTTP body facts across multi-sensor observations and clamp conn byte counters to protocol body floors, enrich Zeek DHCP source-native lease fields, keep OCSP responder choice stable for a certificate identity, render scheduled-task SYSTEM principals as local authority instead of AD-domain users, and derive Sysmon hashes from host OS build when OS binary metadata differs.
