...
- [ ] **IN PROGRESS** Loop 44 realism fixes — address loop-43 concrete root causes: keep Windows Security EventRecordID order monotonic with rendered TimeCreated values, make scheduled-task SYSTEM XML use service-account semantics, bound short-lived Windows utility lifetimes for gpresult/gpupdate/dsquery-like commands, and preserve Zeek same-flow payload bytes across multi-sensor observations while still varying source-native packet/IP counters.
