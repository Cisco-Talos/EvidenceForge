...
- [ ] **IN PROGRESS** Loop 42 realism fixes — align Kerberos PKINIT issuer names with scenario AD org, restrict DHCP baseline leases to DHCP-like client systems instead of static infrastructure, make Linux systemd-logind session IDs monotonic under out-of-order generation, and populate eCAR thread IDs where endpoint telemetry can derive a source thread.
