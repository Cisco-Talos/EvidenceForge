...
- [x] Storyline beacon proxy routing — denied HTTP/S beacon events from hosts with explicit proxy routes now traverse the proxy and emit proxy-denied evidence instead of raw direct client-origin network connections.
...
- [ ] DNS analyzer parity for direct external DNS — blind loop found DNS-labeled external resolver connections and IDS DNS alerts without matching Zeek dns.log rows. Decide whether those flows should get DNS context or whether IDS DNS alerts should only attach where DNS parser evidence exists.
