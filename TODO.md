...
- [ ] Proxy CONNECT flow accounting invariants — successful explicit-proxy CONNECT rows produce Zeek client-proxy connections with implausible accounting (`orig_bytes=1`, `resp_bytes=0`, `orig_pkts=4`, `resp_pkts=0`, `conn_state=SF`) despite Zeek HTTP showing `CONNECT ... 200`. Successful CONNECT negotiation needs responder packets/bytes and plausible request bytes.
- [ ] TLS/SNI-bearing flow accounting invariants — established proxy-origin SSL connections can carry `orig_bytes=0` while ssl.log has SNI, TLS version, cipher, and `established=true`. TLS ClientHello/SNI-bearing sessions need plausible client payload bytes and packets.
- [ ] Proxy byte semantics alignment — proxy access byte columns appear reversed relative to Zeek HTTP `request_body_len`/`response_body_len` for GET rows. Confirm W3C proxy field semantics and align `cs_bytes`/`sc_bytes` consistently across proxy_access and Zeek HTTP.
- [ ] Internal certificate issuer realism — internal `.test` host certificates were issued by public CAs (Let's Encrypt, Google Trust Services, DigiCert). Internal/test domains should use enterprise/private CA or self-signed issuer profiles, not public CA issuers.
- [ ] TLS resumption/certificate-chain realism — resumed TLS sessions should not uniformly carry full certificate chains; first-observed sessions per SNI should usually be non-resumed and later sessions can resume.
- [ ] Proxy edge-case diversity — add more explicit proxy failures and operational noise: auth failures, CONNECT denies, partial tunnels, timeouts/resets, user attribution gaps, and less uniform CONNECT success behavior.
...
- [ ] Configurable cross-source evidence disagreement — deferred by design. Perfect cross-source correlation is useful for training/huntability and should remain the default feature unless a scenario/evaluation profile asks for realism gaps. Later design a deterministic setting for dropped/partial/ambiguous corroborating evidence across Zeek, web, proxy, firewall, IDS, Windows, Sysmon, and eCAR without breaking ground-truth traceability. Include sensor-observation timing realism: the post-CONNECT blind eval found fixed inter-sensor offsets (for example Zeek client/core HTTP/DNS/NTP/weird and core/egress SSL/OCSP differed by a nearly constant 272 microseconds), which should become configurable per-sensor clock/analyzer jitter rather than a one-off HTTP/proxy fix.
