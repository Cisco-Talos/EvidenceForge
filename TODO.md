...
- [ ] Windows process/Sysmon/eCAR blind-eval follow-up from 88% synthetic review — fix critical/high/medium findings recorded after the latest blind eval: Windows Security 5156 overuses PID 4/svchost for browser-like WFP flows and disagrees with eCAR FLOW PID attribution; Security 4688 sometimes parents user processes to PID 4/ntoskrnl.exe; Sysmon Event 7 and eCAR `MODULE/LOAD` are disjoint for overlapping endpoint module-load telemetry; remote-thread joins remain ambiguous when repeated source/target PID pairs appear; and process lifecycle joins may be unrealistically complete. Treat exact cross-source agreement as out of scope unless tied to a concrete technical impossibility.
