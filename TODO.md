...
- [ ] Explicit proxy DENIED requests still produce upstream origin evidence — blind eval found denied proxy records such as `GET http://aws.amazon.com/ 403 ... DENIED` with matching egress HTTP. A denied proxy request should normally stop at the proxy and not emit proxy→origin Zeek HTTP/conn evidence.
- [ ] Explicit proxy client-side origin DNS too common — blind eval found workstation-side DNS for many origin web domains. In explicit proxy mode, most origin resolution should happen on the proxy, with only controlled exceptions for prefetch/leakage/direct non-web traffic.
- [ ] Explicit proxy TLS completeness gaps — blind eval found established proxy→origin `service=ssl` connections without corresponding `ssl.log`/x509 evidence. Ensure successful TLS proxy-origin legs consistently emit SSL/x509 records while preserving plausible failures.
- [ ] Explicit proxy CONNECT reuse/pooling realism — blind eval found mechanically high per-host CONNECT counts for repeated destinations. Model tunnel/session reuse or pooling so proxy access and Zeek connection counts are less one-object/one-tunnel regular.
