...
- [ ] **IN PROGRESS** Loop 35 iteration-test assessment continuation — fix loop-34 root-cause findings: Sysmon Event 1 `CurrentDirectory` was universally defaulting to `C:\Windows\system32\`, local Linux failed logons rendered impossible `sshd ... from - port` messages, repeated workstation 4800 locks could occur before a 4801 unlock for the same visible session, and PsExec-style follow-on commands flattened directly under `services.exe` instead of a live remote execution wrapper. Focused regression tests for these root-cause fixes are passing; full normal pytest, Ruff, commit, regenerate/eval/probe/blind-review still pending.
